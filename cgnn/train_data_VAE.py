...
class CVAE(tf.keras.Model):
    def __init__(self, sensitivity_file, min_model, max_model, norm_pad=0.1,
                 img_rows=32, img_cols=32, channels=1, n_data=100, data_std=1,
                 model_std=1, latent_dim=50, beta_vae=1, model_loss_type='mse',
                 data_loss_type='mse'):
        ...
        if model_loss_type == 'se':
            self.model_mean_error = tf.keras.losses.MeanSquaredError(
                reduction=tf.keras.losses.Reduction.NONE)
            self.model_weights = (img_rows*img_cols)/(model_std**2)
        if model_loss_type == 'mse':
            self.model_mean_error = tf.keras.losses.MeanSquaredError(
                reduction=tf.keras.losses.Reduction.NONE)
            self.model_weights = 1/(model_std**2)
        if model_loss_type == 'ae':
            self.model_mean_error = tf.keras.losses.MeanAbsoluteError(
                reduction=tf.keras.losses.Reduction.NONE)
            self.model_weights = (img_rows*img_cols)/model_std
        ...
        if data_loss_type == 'se':
            self.data_mean_error = tf.keras.losses.MeanSquaredError(
                reduction=tf.keras.losses.Reduction.NONE)
        ...
        if data_loss_type == 'ae':
            self.data_mean_error = tf.keras.losses.MeanAbsoluteError(
                reduction=tf.keras.losses.Reduction.NONE)
    ...
    def predict(self, model, zero_first_datum=False,
                subtract_model_mean=False):
        '''
        Predict data, given a model
        if zero_first_datum, shift all data such that first datum is 0
        if subtract_model_mean, remove the model mean before modeling
            (density contrast)
        ...
        if subtract_model_mean:
            m64 -= tf.expand_dims(tf.expand_dims(tf.reduce_mean(
                m64, axis=(1, 2)), axis=1), axis=2)
        data = tf.linalg.einsum('ijk,ljk...->li...', self.sensitivity, m64)
        if zero_first_datum:
            data -= data[:, 0:1, ...]
        return data

    def predict_tanh(self, tanhs, zero_first_datum=False,
                     subtract_model_mean=False):
        '''
    ...
    def plot_data(self, save2file=False, folder='.', samples=16,
                  latent=None, step=None, ylims=(-1.5,1.5)):
        if latent is None:
            latent = np.random.normal(
                0.0, 1.0, size=[samples, self.latent_dim+self.n_data])
        if step is None:
...
def plot_images(images, save2file=False, filename='./model.png', step=None):
...
def plot_lines(data, save2file=False, filename='./gravity.png', step=None,
               ylims=(-1.5,1.5)):
    plt.figure(figsize=(10, 10))
...
def compute_loss(network, x):
    '''
    total loss function
    TODO: incorporate data as input label
    '''
    ...
    data_misfit = network.data_mean_error(
        d_true, d_pre, sample_weight=network.data_weights)
    # print(x_tanh.shape, x.shape)
    logpx_z = network.model_mean_error(
        tf.reshape(x_tanh, (-1, network.img_rows*network.img_cols)),
        tf.reshape(x, (-1, network.img_rows*network.img_cols)),
        # sample_weight=(network.img_rows*network.img_cols)/(network.model_std**2))
    ...
    return -tf.reduce_mean(-data_misfit - logpx_z +
...
def compute_reconstruction_loss(network, x):
    ...
    logpz = log_normal_pdf(z, 0., 0.)
    logqz_x = log_normal_pdf(z, mean, logvar)
    return -tf.reduce_mean(-logpx_z + network.beta_vae*(logpz - logqz_x))


@tf.function
def compute_apply_gradients(network, x, optimizer, use_data_misfit=True):
